...
const UserCreationModal = ({ isOpen, onRequestClose }) => {
  ...
  const [canReopenRequest, setCanReopenRequest] = useState(false);
  const { token } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_ENDPOINT}/Auth/register/${role === 'A' ? 'admin' :
        ...
        { firstName, lastName, "email": email + "@elastomer-solutions.com", code, role, departement, "reOpenAfterValidation": canReopenRequest },
        {
          headers: {
      ...
      <form onSubmit={handleSubmit}>
        ...
        {role === 'P' && (
          <div className="mb-4 flex items-center">
            <label htmlFor="canReopenRequest" className="block mb-2 text-sm font-medium text-gray-900 mr-2">Can Reopen Request After Validation</label>
            <input
              type="checkbox"
              id="canReopenRequest"
              checked={canReopenRequest}
              onChange={(e) => setCanReopenRequest(e.target.checked)}
