...
const fetchAllUsers = async (token) => {
  const res = await fetch(`${process.env.REACT_APP_API_ENDPOINT}/Users/users`, {
    method: 'GET',
    headers: {
...
const departments = ['All', 'IT', 'RH', 'MAINT', 'DE']; // Define your departments here
const roles = ['All', 'Admin', 'Purchaser', 'Requesteur', 'Validator']; // Define your roles here
...
function Dashboard() {
  ...
  const { data: users, isLoading, error } = useQuery(["users", auth.token], () => fetchAllUsers(auth.token), {
    enabled: !!auth.token,
  });
  ...
  useEffect(() => {
    if (users) {
      ...
      if (selectedDepartment !== 'All') {
        filtered = filtered.filter(user => user.departement === selectedDepartment);
      }
  ...
  }, [users, selectedDepartment, selectedRole]);
  ...
  return (
    <div className="p-4 sm:ml-64">
      <div className="p-4 border-2 border-gray rounded-lg" style={{ marginTop: 130 }}>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button className="p-2 rounded-full group transition-all duration-500 flex item-center">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path className="stroke-black " d="M10.0161 14.9897V15.0397M10.0161 9.97598V10.026M10.0161 4.96231V5.01231" stroke="black" strokeWidth="2.5" strokeLinecap="round"></path>
                                </svg>
                              </button>
                            </div>
                          </td>
